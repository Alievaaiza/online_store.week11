...
class OrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = (
            'items', 'total_cost', 'delivery_address', 'contact_number',
            'first_name', 'status'
        )

    def create(self, validated_data):
        # {'items': [OrderedDict([('product', < Product: Product1 >), ('quantity', 5)]),
        #            OrderedDict([('product', < Product: Product 2 >), ('quantity', 3)])], 'total_cost': 0,
        #  'delivery_address': 'Bishkek', 'contact_number': '+996700939511', 'first_name': 'Tom'}
